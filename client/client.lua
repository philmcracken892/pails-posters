        ...
        for i = 1, #SpawnedProps do
            ...
            if prop and prop.obj and DoesEntityExist(prop.obj) then
                ...
                if distance <= (Config.TextDisplayDistance or 10.0) then
                    DrawText3D(propPos.x, propPos.y, propPos.z + 0.3, "Notice")
                end
            end
        end
    end
...
RegisterNetEvent('rex-notes:client:opennotes', function(noteid, entity)
    ...
    RSGCore.Functions.TriggerCallback('rex-notes:server:isAdmin', function(isAdmin)
        local menuOptions = {
            {
                title = '📖 Read Poster',
...
---------------------------------------------
RegisterNetEvent('rex-notes:client:setupnote', function(proptype, PropHash, pPos, rotation)
    local input = lib.inputDialog('Create Note', {
        ...
        { 
            ...
            description = 'Discord image URL or any direct image link',
            required = false
        }
    })
    ...
    if imageUrl and imageUrl ~= "" then
        if not IsValidImageURL(imageUrl) then
            lib.notify({ 
                ...
                description = 'Please provide a valid image URL (Discord links or direct image URLs)',
                type = 'error', 
                duration = 5000 
            })
            return
    ...
    TriggerEvent('rex-notes:client:placeNewProp', proptype, PropHash, pPos, rotation, input[1], input[2], imageUrl)
end)
...
RegisterNetEvent('rex-notes:client:readnote', function(data)
    RSGCore.Functions.TriggerCallback('rex-notes:server:getallpropdata', function(result)
        ...
        local title = noteData.title or 'Untitled Note'
        local content = noteData.note or 'No content available.'
...
RegisterNetEvent('rex-notes:client:readcopiednote', function(noteid, title, content, imageUrl)
    -- Set focus and show custom UI
    SetNuiFocus(true, true)
    SendNUIMessage({
        action = 'showNote',
...
RegisterNetEvent('rex-notes:client:editnote', function(data)
    -- First fetch current note data from the server
    RSGCore.Functions.TriggerCallback('rex-notes:server:getallpropdata', function(result)
        if not result or not result[1] then
            lib.notify({
                title = 'Error',
                description = 'Could not load poster data for editing',
                type = 'error',
                duration = 5000
            })
            return
        ...
        local input = lib.inputDialog('Edit Poster', {
            { type = 'input', label = 'Title', default = currentTitle, required = true },
            { type = 'textarea', label = 'Content', default = currentContent, autosize = true, required = true },
            { type = 'input', label = 'Image URL (Optional)', default = currentImageUrl }
        })
        ...
        if newImageUrl and newImageUrl ~= "" then
            if not IsValidImageURL(newImageUrl) then
                lib.notify({
                    title = 'Invalid Image URL',
                    description = 'Please provide a valid direct image link or Discord link',
                    type = 'error',
                    duration = 5000
                })
                return
        ...
        TriggerServerEvent('rex-notes:server:editnote', data.noteid, input[1], input[2], newImageUrl)
    end, data.noteid)
end)
...
AddEventHandler('rex-notes:client:placeNewProp', function(proptype, pHash, pos, rotation, title, note, imageUrl)
    RSGCore.Functions.TriggerCallback('rex-notes:server:countprop', function(result)

        if result > Config.MaxNotes then
            lib.notify({ title = 'Max Posters Reached', type = 'error', duration = 7000 })
            return
        end
        ...
        if not IsPedInAnyVehicle(PlayerPedId(), false) and not isBusy then
            ...
            ClearPedTasks(cache.ped)
            FreezeEntityPosition(cache.ped, false)
            TriggerServerEvent('rex-notes:server:newProp', proptype, pos, rotation, pHash, title, note, imageUrl)
            isBusy = false
            return
        else
